									...
									<table class="table table-hover">
										...
										<?php $no=1; foreach($admin as $a) { ?>
											<tr>
												...
												<td><?= $a->nama; ?></td>
												<td><?= $a->email; ?></td>
												<td><span class="badge badge-success px-2"><?= $a->status; ?></span>
												</td>
                                                <td class="color-primary"><?= date('d M Y',$a->created_at) ?></td>
												<td class="text-center"><a class="btn btn-sm btn-danger" href="<?=base_url('admin/delete_admin');?>/<?= $a->id_admin ?>" onClick="return confirm('Yakin?');"><i class="fa fa-fw fa-trash"></i></a></td>
